...
* **Simple is Best (Relentless Simplicity):** Prioritize writing the cleanest, most efficient code and the best possible algorithms while keeping the codebase simple, readable, and elegant. Every single line of generated code must be continually cross-examined to avoid over-engineering.
...
* **More with Less (No Bloated Code):** Do not overcomplicate or bloat files into hundreds of lines. Prioritize modularizing logic into small, reusable, and easily manageable modules, helper functions, custom hooks, and components. Always verify that files remain strictly modularized and check that you have not accidentally introduced bloated code when implementing any specific instruction or function. Refer to [Architecture.md](file:///C:/AI-Integrated-Coding/INFOMAN%20PROJECT/Architecture.md#8-project-file-line-counts-anti-bloat-tracker) to consult the line counts of all active files and audit for bloat. Note: This constraint applies to all application source and execution code (e.g., TypeScript, JavaScript, Python, TSX) and is not enforced for `.md` files or other meta-documentation assets.
